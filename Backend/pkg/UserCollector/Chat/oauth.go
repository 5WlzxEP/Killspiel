...
func (tc *TwitchChat) setOAuthStandard(ctx *fiber.Ctx) error {
	...
	return ctx.Redirect(fmt.Sprintf("https://id.twitch.tv/oauth2/authorize?response_type=token&client_id=6yqxsptsgw5bffjno9b1q9dtr2nlf8&redirect_uri=%s/oauth/&scope=chat:read+chat:edit+channel:moderate+whispers:read+whispers:edit+moderator:manage:announcements+moderation:read", ctx.BaseURL()), http.StatusTemporaryRedirect)
}
